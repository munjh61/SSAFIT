    ...
    <div class="new">
        ...
        <div class="auth-button">
            <div class="two-button-box">
                <button v-if="step < 4" style="width: 49%;" @click="stepDown">이전</button>
                <button v-if="step < 4" style="width: 49%;" @click="stepUp">다음</button>
            </div>
            <button v-if="step == 4" @click="regist">회원가입</button>
        </div>
    </div>
</template>
...
// 회원가입
const regist = () => {
    if(checkUserId.value && checkPassword.value && checkEmail.value){
        const result = store.regist(userId.value, password.value, email.value, userName.value)
        if(result){
            alert(store.msg)
            emit('close')
        }
